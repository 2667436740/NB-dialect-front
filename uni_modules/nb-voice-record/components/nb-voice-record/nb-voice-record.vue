...
export default {
	...
	methods: {
		...
		async checkPermission() {
			// #ifdef APP-PLUS
			// 先判断os
			let os = await uni.getSystemInfoSync().osName;
			let state;
			if (os == 'ios') {
				state = permision.judgeIosPermission('record');
			} else {
				state = await permision.requestAndroidPermission('android.permission.RECORD_AUDIO');
				state = state == 1 ? true : state == 0 ? false : state == -1 ? 'reject' : 'unknow';
			}
